...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        int sumStock = 0;
        for (int i = 0; i < products.length; i++) {
            sumStock += prices[i] * counts[i];
        }
        boolean doBonus = sumStock >= MIN_COST_FOR_BONUS;
        for (int i = 0; i < products.length; i++) {
            if (counts[i] != 0) {
                ...
                if (isOnSale) {
                    System.out.println("\t" + products[i] + " " + (doBonus ? counts[i] + 1 : counts[i]) + " шт. за " + (prices[i] * (counts[i] / 3 * 2 + counts[i] % 3)) + " руб. (Акция!)");
